...
`setup_logging()` 在 `main()` 启动时调用一次（幂等守卫防止重复 handler）。日志格式：`时间 级别 模块名 函数:行号 消息`。控制台输出 INFO 级别，文件 `wiki-rag.log` 记录 DEBUG 级别。各模块通过 `logging.getLogger(__name__)` 获取已配置的 logger。
...
- **向量存储**: Milvus local 模式（`./milvus_wiki.db`），IVF_FLAT 索引 + COSINE 相似度。支持 remote 模式连接独立 Milvus 服务。**已知问题**：Windows 下 MilvusLite 进程崩溃后 LOCK 文件残留，需手动删除 `milvus_wiki.db/` 目录后重启
- **同步**: 增量模式通过 MD5(`content|mtime`) 哈希对比变更，元数据写入目标目录的 `.wiki_sync_metadata.json`。`sync()` 返回 `SyncPlan`（含 `new_files/updated_files/deleted_files/total_chunks_estimate`），实际 chunk 数在同步完成后覆盖 estimate
...
- **测试覆盖**: 37 个测试，87% 覆盖率。未覆盖部分主要是 Ollama/OpenAI 实现（需外部服务）和 BM25 预留接口
- **日志**: 所有核心模块已接入日志，记录启动/工具调用/同步变更/检索耗时/API 错误。日志文件 `wiki-rag.log`

## 测试

```bash
pytest tests/ -v                                   # 全部 37 个测试
pytest tests/test_main.py -v                       # main.py 构建逻辑 (4 个)
pytest tests/test_chunker.py -v                    # 分块器 (8 个)
pytest tests/test_embedder.py -v                   # 嵌入器 (4 个)
pytest tests/test_logger.py -v                     # 日志配置 (4 个)
...
pytest tests/test_sync_strategy.py -v              # 同步管理器 (7 个)
```
